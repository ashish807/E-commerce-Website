...
from accounts.models import AddAddress, AddState
...
def add_cart(request, product_id):
    ...
    if current_user.is_authenticated:
        ...
        if 'cart' in request.POST:
            ...
            return redirect('buy_checkout_guest')
        else:
            return redirect(url) 
...
def checkout(request, total=0, quantity=0, cart_items=None):
    address =  AddAddress.objects.filter(user = request.user).exists()
    tax=0
    grand_total=0
    try:
        if request.user.is_authenticated:
...
@login_required(login_url = 'login')
def buy_checkout(request,total=0, quantity=0, cart_items=None):
    tax=0
    grand_total=0
    address =  AddAddress.objects.filter(user = request.user).exists()
...
def buy_checkout_guest(request,total=0, quantity=0, cart_items=None):
    tax=0
    grand_total=0
    ...
    return render(request, 'store/checkout_guest.html', context)


def checkout_guest(request,total=0, quantity=0, cart_items=None):
    tax=0
    grand_total=0
    try:
        if request.user.is_authenticated:
